...
const form = useForm({
    _method: "PUT",
    name: props.conductor.name,
    employee_id: props.conductor.employee_id,
    phone: props.conductor.phone,
    email: props.conductor.email,
    address: props.conductor.address,
    status: props.conductor.status,
    image: null,
});

const imagePreview = ref(props.conductor.image_url);
...
const submit = () => {
    form.post(route("admin.conductors.update", props.conductor.id), {
        preserveScroll: true,
        forceFormData: true,
    });
};
...
<template>
    <Head title="Edit Kondektur" />

    <AdminLayout title="Edit Kondektur">
