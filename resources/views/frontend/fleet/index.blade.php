...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 page-spacing">
        ...
        <div class="text-center mb-10">
            ...
            <p class="text-lg text-gray-600 max-w-2xl mx-auto">Nikmati perjalanan yang nyaman dan aman dengan armada bus
                modern kami dan pengemudi profesional</p>
        </div>
        ...
        <div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl shadow-lg p-6 mb-10">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @forelse($buses as $bus)
                    <div
                        class="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2">
                        @if ($bus->getFirstMediaUrl('buses'))
                            <div class="relative">
                                <img src="{{ $bus->getFirstMediaUrl('buses') }}" alt="{{ $bus->name }}"
                                    class="w-full h-56 object-cover">
                                <div
                                    class="absolute top-4 right-4 bg-blue-600 text-white px-3 py-1 rounded-full text-sm font-bold">
                                    {{ $bus->bus_type ?? 'Standar' }}
                                </div>
                            </div>
                        @else
                            <div
                                class="relative bg-gradient-to-r from-gray-200 to-gray-300 w-full h-56 flex items-center justify-center">
                            ...
                            <div class="flex justify-between items-start mb-3">
                                ...
                                <div class="bg-green-100 text-green-800 px-2 py-1 rounded-full text-xs font-bold">
                                    <img src="{{ asset('img/car-seat.png') }}" alt="Seat"
                                        class="w-4 h-4 mr-1 inline-block">{{ $bus->capacity }} kursi
                                </div>
                            </div>
                            <p class="text-gray-600 mb-4">
                                {{ $bus->description ?? 'Tidak ada deskripsi tersedia untuk bus ini.' }}</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @forelse($drivers as $driver)
                    <div
                        ...
                        <div class="relative">
                            <div
                                class="bg-gradient-to-r from-gray-200 to-gray-300 w-full h-56 flex items-center justify-center">
                                @if ($driver->getFirstMediaUrl('drivers'))
                                    <img src="{{ $driver->getFirstMediaUrl('drivers') }}" alt="{{ $driver->name }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                @forelse($facilities as $facility)
                    <div
                        ...
                        @else
                            <div
                                class="bg-gray-200 border-2 border-dashed rounded-xl w-16 h-16 mx-auto mb-4 flex items-center justify-center">
