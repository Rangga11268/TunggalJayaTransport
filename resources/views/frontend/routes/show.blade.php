        ...
        <div class="mobile-info-card-content">
            ...
            <div class="md:hidden space-y-4">
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                    <div class="p-4 border-b border-gray-200 bg-gradient-to-r {{ $schedule->is_weekly ? 'from-green-50 to-emerald-50' : 'from-blue-50 to-indigo-50' }}">
                        <div class="flex justify-between items-center">
                            <div class="flex items-center">
                                <div class="flex-shrink-0 h-8 w-8 rounded-full flex items-center justify-center {{ $schedule->is_weekly ? 'bg-green-100' : 'bg-blue-100' }}">
                                    <i class="fas fa-{{ $schedule->is_weekly ? 'calendar-week' : 'clock' }} text-{{ $schedule->is_weekly ? 'green' : 'blue' }}-600 text-sm"></i>
                                </div>
                                <div class="ml-2">
                                    <div class="text-sm font-medium text-gray-900">{{ $schedule->is_weekly ? 'Weekly' : 'Daily' }}</div>
                                </div>
                            </div>
                            <div class="text-xs font-medium {{ $schedule->hasDeparted() ? 'text-red-600' : 'text-gray-500' }}">
                                @if($schedule->hasDeparted())
                    ...
                    <div class="p-4">
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <div class="flex items-center mt-1">
                                    ...
                                    <div>
                                        <div class="text-sm font-medium text-gray-900 truncate">{{ $schedule->bus->name ?? $schedule->bus->bus_type ?? 'Standard' }}</div>
                            ...
                            <div>
                                ...
                                <div class="text-lg font-bold text-gray-900 mt-1">Rp. {{ number_format($schedule->price, 0, ',', '.') }}</div>
                            </div>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            @else
                                <div class="flex items-center">
                                    <div class="text-sm text-gray-900 mr-2">{{ $schedule->getAvailableSeatsCount() }} / {{ $schedule->bus->capacity }} seats</div>
                        ...
                        <div class="pt-2">
                            @if($schedule->hasDeparted())
                                <button class="w-full inline-flex items-center justify-center px-4 py-2 bg-gray-300 border border-transparent rounded-md font-semibold text-gray-700 cursor-not-allowed text-sm">
                                    Departed
                                </button>
                            @elseif($schedule->getAvailableSeatsCount() > 0 && $schedule->isAvailableForBooking())
                                <a href="{{ route('frontend.booking.show', $schedule->id) }}" class="w-full inline-flex items-center justify-center px-4 py-2 bg-gradient-to-r from-green-600 to-emerald-700 border border-transparent rounded-md font-semibold text-white hover:from-green-700 hover:to-emerald-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 shadow-sm transition duration-300 text-sm">
                                    <i class="fas fa-ticket-alt mr-1"></i>Book Now
                                </a>
                            @elseif($schedule->getAvailableSeatsCount() == 0)
                                <button class="w-full inline-flex items-center justify-center px-4 py-2 bg-gray-300 border border-transparent rounded-md font-semibold text-gray-700 cursor-not-allowed text-sm">
