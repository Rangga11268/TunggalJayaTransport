                                    ...
                                    <tr>
                                        ...
                                        <td class="px-4 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex flex-col sm:flex-row gap-2">
                                                <a href="{{ route('admin.news.edit', $article) }}" class="text-indigo-600 hover:text-indigo-900 text-center touch-friendly">Edit</a>
                                                <form id="delete-form-{{ $article->id }}" action="{{ route('admin.news.destroy', $article) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" class="text-red-600 hover:text-red-900 w-full text-center touch-friendly" onclick="handleDelete('delete-form-{{ $article->id }}', 'Hapus Artikel?', 'Apakah Anda yakin ingin menghapus artikel ini? Tindakan ini tidak dapat dibatalkan.')">Hapus</button>
                                                </form>
                                            </div>
                                        </td>
                                    </tr>
