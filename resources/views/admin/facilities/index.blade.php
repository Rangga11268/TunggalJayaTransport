                                ...
                                @forelse($facilities as $facility)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <a href="{{ route('admin.facilities.edit', $facility) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">Edit</a>
                                            <form id="delete-form-{{ $facility->id }}" action="{{ route('admin.facilities.destroy', $facility) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" class="text-red-600 hover:text-red-900" onclick="handleDelete('delete-form-{{ $facility->id }}', 'Hapus Fasilitas?', 'Apakah Anda yakin ingin menghapus fasilitas ini? Tindakan ini tidak dapat dibatalkan.')">Hapus</button>
                                            </form>
                                        </td>
                                    </tr>
                                @empty
