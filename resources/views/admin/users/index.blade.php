                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            ...
                                            @if($user->id != auth()->id())
                                                <form id="delete-form-{{ $user->id }}" action="{{ route('admin.users.destroy', $user) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" class="text-red-600 hover:text-red-900" onclick="handleDelete('delete-form-{{ $user->id }}', 'Hapus Pengguna?', 'Apakah Anda yakin ingin menghapus pengguna ini? Tindakan ini tidak dapat dibatalkan.')">Hapus</button>
                                                </form>
                                            @endif
                                        </td>
                                    </tr>
