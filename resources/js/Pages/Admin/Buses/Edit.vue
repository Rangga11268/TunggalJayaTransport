...
const form = useForm({
    _method: "PUT", // For handling file upload with PUT in Laravel
    name: props.bus.name,
    ...
    drivers: props.bus.drivers ? props.bus.drivers.map((d) => d.id) : [],
    conductors: props.bus.conductors
        ? props.bus.conductors.map((c) => c.id)
        : [],
    image: null,
});

const imagePreview = ref(props.bus.image_url);
...
const submit = () => {
    form.post(route("admin.buses.update", props.bus.id), {
        preserveScroll: true,
        onError: () => {
...
const availableDrivers = computed(() => {
    // Show unassigned drivers OR drivers currently assigned to THIS bus
    const currentDriverIds = form.drivers;
    return props.drivers.filter(
        (driver) =>
            !props.assignedDrivers.includes(driver.id) ||
            currentDriverIds.includes(driver.id)
    );
});

const availableConductors = computed(() => {
    // Show unassigned conductors OR conductors currently assigned to THIS bus
    const currentConductorIds = form.conductors;
    return props.conductors.filter(
        (conductor) =>
            !props.assignedConductors.includes(conductor.id) ||
            currentConductorIds.includes(conductor.id)
    );
});
</script>
