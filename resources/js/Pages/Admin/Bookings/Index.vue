...
const { selectedIds, selectAll } = useBulkDelete(localBookings);
...
const bulkDelete = () => {
    ...
    Swal.fire({ title: `Hapus ${selectedIds.value.length} booking?`, text: "Data yang dihapus tidak dapat dikembalikan!", icon: "warning",
        ...
        axios.post(route("admin.bookings.bulk-destroy"), { ids: selectedIds.value, _method: "DELETE" })
            .then(() => {
                Swal.fire({ icon: "success", title: "Berhasil!", text: `${selectedIds.value.length} booking dihapus.`, timer: 1500, showConfirmButton: false });
                localBookings.value = { ...localBookings.value, data: localBookings.value.data.filter(d => !selectedIds.value.includes(d.id)), total: localBookings.value.total - selectedIds.value.length };
                selectedIds.value = [];
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route("admin.bookings.destroy", id), {
                preserveScroll: true,
                onSuccess: () => {
                    localBookings.value = { ...localBookings.value, data: localBookings.value.data.filter(d => d.id !== id), total: localBookings.value.total - 1 };
        ...
        >
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    >
                        <tr v-for="booking in localBookings?.data" :key="booking.id"
                            class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors"
                            :class="{'bg-brand-red/5': selectedIds.includes(booking.id)}">
                            <td class="px-4 py-4">
                                <input type="checkbox" :value="booking.id" v-model="selectedIds" class="w-4 h-4 rounded border-gray-300 text-brand-red focus:ring-brand-red cursor-pointer" />
                            </td>
